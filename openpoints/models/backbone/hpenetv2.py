...
from ..layers import create_convblock1d, create_convblock2d, create_act, CHANNEL_MAP, \
    create_grouper, furthest_point_sample, random_sample, three_interpolation, get_aggregation_feautres, HPE, grouping_operation, BAFM
...
class LocalAggregation(nn.Module):
    ...
    def forward(self, pf) -> torch.Tensor:
        ...
        #f_raw = f
        f = self.convs(f)
        fj = grouping_operation(f, idx_dp)
        f = self.pool(fj+pe) #+ f_raw
...
class SetAbstraction(nn.Module):
    ...
    def __init__(self,
        ...
        create_conv = create_convblock2d if self.convs_d else create_convblock1d
        convs = []
        ...
        if not is_head:
            ...
            if self.convs_d:
                self.rel_pos = HPE(in_channels=channels[0], mode=1, norm_args=norm_args, act_args=act_args)
            else:
                self.rel_pos = HPE(in_channels=channels[-1], mode=1, norm_args=norm_args, act_args=act_args)             
...
class InvResMLP_block(nn.Module):
    def __init__(self,
                 in_channels,
                 norm_args=None,
                 act_args=None,
...
class InvResMLP(nn.Module):
    def __init__(self,
        ...
        for i in range(1, blocks):
            layers.append(InvResMLP_block(self.in_channels,
                                ...
                                norm_args=self.norm_args, act_args=self.act_agrs, group_args=group_args,
                                conv_args=self.conv_args, expansion=self.expansion,
                                use_res=self.use_res
                                ))
        self.layer = nn.Sequential(*layers)
        self.rel_pos = HPE(in_channels=in_channels, mode=1, norm_args=norm_args, act_args=act_args)
    ...
    def forward(self, pf):
        ...
        pe = self.rel_pos(dp)
        p, f, dp, idx_dp, pe = self.layer([p, f, dp, idx_dp, pe])
        return p, f
...
class HPENetV2Encoder(nn.Module):
    ...
    def __init__(self,
                 ...
                 convs_d: List[bool] = [False, True, False, False, False],
        ...
        for i in range(len(blocks)):
            ...
            encoder.append(self._make_enc(
                block, channels[i], blocks[i], stride=strides[i], convs_d=convs_d[i], group_args=group_args,
    ...
    def _make_enc(self, block, channels, blocks, stride, convs_d, group_args, is_head=False):
        layers = []
        ...
        layers.append(SetAbstraction(self.in_channels, channels,
                                     self.sa_layers if not is_head else 1, stride, convs_d,
...
@MODELS.register_module()
class HPENetV2Decoder(nn.Module):
    def __init__(self,
        ...
        for i in range(-1, -n_decoder_stages - 1, -1):
            '''
            if i ==-n_decoder_stages:
                self.ddfc2 = BAFM(skip_channels[i+1]*2, skip_channels[i+1], ration=2, norm_args=norm_args, act_args=act_args)
                self.fc2 = create_convblock1d(skip_channels[i+1]*2, skip_channels[i+1], norm_args=None, act_args=act_args)
                self.fc_all2 = create_convblock1d(skip_channels[i+1]*2, skip_channels[i+1], norm_args=None, act_args=act_args)
            '''
            decoder[i] = self._make_dec(skip_channels[i], fp_channels[i])
        self.decoder = nn.Sequential(*decoder)
        self.out_channels = fp_channels[-n_decoder_stages]*2
        self.ddfc = BAFM(skip_channels[0]*2, skip_channels[0], ration=4, norm_args=None, act_args=act_args)
        self.fc_all = create_convblock1d(skip_channels[0]*2, skip_channels[0], norm_args=None, act_args=act_args)
    ...
    def forward(self, p, f):
        ...
        f[3] = self.decoder[2][1:]([p[3], self.decoder[2][0]([p[3], f[3]], [p[4], f[4]])])[1]
        f[2] = self.decoder[1][1:]([p[2], self.decoder[1][0]([p[2], f[2]], [p[3], f[3]])])[1]
        #f3_new = self.fc_all2(f[3])
        #f3_new = three_interpolation(p[2], p[3], f3_new)
        #f2_mid = self.fc2(self.ddfc2(f3_new,f[2]))
        f2_mid = f[2]

        f[1] = self.decoder[0][1:]([p[1], self.decoder[0][0]([p[1], f[1]], [p[2],f2_mid])])[1]
        f2_new = self.fc_all(f2_mid)
        f2_new = three_interpolation(p[1], p[2], f2_new)
...
@MODELS.register_module()
class HPENetV2PartDecoder(nn.Module):
    def __init__(self,
                 encoder_channel_list: List[int],
                 decoder_layers: int = 2,
        ...
        self.decoder = nn.Sequential(*decoder)
        self.out_channels = fp_channels[-n_decoder_stages]*2
        self.ddfc = BAFM(fp_channels[0]*2, fp_channels[0], ration=4, norm_args=None, act_args=act_args)
        self.fc_all = create_convblock1d(fp_channels[0]*2, fp_channels[0], norm_args=None, act_args=act_args)
    ...
    def forward(self, p, f, cls_label):
        ...
        #f_mid = f[-len(self.decoder) - 1]
        # TODO: study where to add this ? 
        f[1] = torch.cat([cls_one_hot, f[1]], 1)
        f[-len(self.decoder) - 1] = self.decoder[0][1:](
            [p[1], self.decoder[0][0]([p[1], f[1]], [p[2], f[2]])])[1]
        
        f2_new = self.fc_all(f[2])
        f2_new = three_interpolation(p[1], p[2], f2_new)
        f_mid = self.ddfc(f2_new,f[1])
