...
#ifndef TENSORRT_C_API_H
#define TENSORRT_C_API_H
...
extern "C" {
      ...
      */
    typedef struct TensorrtInferencePipeline_C TensorrtInferencePipeline_C;
    ...
    /**
     * @brief 对包含多个雷达为一帧的点云数据直接执行推理（不经过 PLY 文件读写）。
     *
     * @param handle       有效的推理管线句柄。
     * @param coord_x      点云 X 坐标数组（长度 n）。
     * @param coord_y      点云 Y 坐标数组（长度 n）。
