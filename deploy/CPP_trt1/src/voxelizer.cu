// ===========================================================================
//  GPU function from src/kernels/voxelize.cu ¡ª declared in voxelizer.h
// ===========================================================================
...
//    1. Upload coordinates ¡ú GPU
//    2. Compute FNV hashes + thrust::sort_by_key   (GPU)
//    3. Download sorted hashes + indices            (GPU ¡ú CPU)
//    4. Scan sorted hashes ¡ú unique + counts        (CPU)
    ...
    int seed)
{
    ...
    if (err) {
        free_all(); throw std::runtime_error("Voxelizer cudaMalloc coord: "
            + std::string(cudaGetErrorString(err)));
    }
    err = cudaMalloc(&d_hashes, hash_bytes);
    if (err) {
        free_all(); throw std::runtime_error("Voxelizer cudaMalloc hashes: "
            + std::string(cudaGetErrorString(err)));
    }
    err = cudaMalloc(&d_indices, index_bytes);
    if (err) {
        free_all(); throw std::runtime_error("Voxelizer cudaMalloc indices: "
            + std::string(cudaGetErrorString(err)));
    }
    ...
    if (err) {
        free_all(); throw std::runtime_error("Voxelizer cudaMemcpy coord: "
            + std::string(cudaGetErrorString(err)));
    }
    ...
    if (err) {
        free_all(); throw std::runtime_error("Voxelizer cudaMemcpy hashes: "
            + std::string(cudaGetErrorString(err)));
    }
    err = cudaMemcpy(indices_sorted.data(), d_indices, index_bytes, cudaMemcpyDeviceToHost);
    if (err) {
        free_all(); throw std::runtime_error("Voxelizer cudaMemcpy indices: "
            + std::string(cudaGetErrorString(err)));
    }
    ...
    // Walk through sorted hashes ¡ª each hash change = new voxel
    ...
    // Single RNG instance ¡ª state advances through all shuffles
...
// ===========================================================================
// launch_voxelize_sort  ¡ª  GPU hash + sort pipeline
//
...
// implicitly serialised ¡ª no explicit synchronisation needed.
// ===========================================================================
