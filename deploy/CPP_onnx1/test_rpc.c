...
static int test_rdp_ai_full_flow(const char* onnx_path, const char* stats_path) {
    ...
    // 4. 执行推理
    float latency = rdp_ai_infer_and_update(
    ...
    // 7. 测试空数据
    printf("\n--- Edge case: empty CDI ---\n");
    latency = rdp_ai_infer_and_update(
...
int main(int argc, char* argv[]) {
    ...
    if (argc >= 3) {
        ...
        OnnxInferencePipeline_C* pipe = onnx_pipeline_create(argv[1], argv[2]);
        if (pipe) {
            printf("[PASS] Pipeline created\n");

            if (argc >= 5) {
                float latency = onnx_pipeline_process_file(argv[3], argv[4]);
                if (latency >= 0) {
    ...
    if (argc >= 3) {
        ...
        printf("\n[SKIP] rdp_ai_full_flow test (usage: %s <model.onnx> <stats.json> [input.ply output.ply])\n",
               argv[0]);
        printf("       Offset verification only (no ONNX model loaded).\n");
    }
