...
static OnnxInferencePipeline_C* g_handle = nullptr;
// ── 不透明结构体定义 ─────────────────────────────────────────────────────
// 这个结构体只在 .cpp 文件中定义，对 C 侧完全透明
struct OnnxInferencePipeline_C {
    std::unique_ptr<OnnxInferencePipeline> cpp_pipeline;
...
extern "C" {
    ...
    DEPLOYAI_LIB_API OnnxInferencePipeline_C* onnx_pipeline_create(
        ...
        try {
            auto handle = std::make_unique<OnnxInferencePipeline_C>();

            handle->cpp_pipeline = std::make_unique<OnnxInferencePipeline>(
                std::string(onnx_path),
                std::string(stats_json_path)
            );
            //释放所有权，将裸指针
            g_handle = handle.release();
            return g_handle;
        }
    ...
    DEPLOYAI_LIB_API void onnx_pipeline_destroy(void) {
        if (!g_handle) return;          // 传入的指针本身为 NULL
        //OnnxInferencePipeline_C* p = *handle;
        //if (!p) return;               // *handle 已经为 NULL
        {
            std::lock_guard<std::mutex> lock(g_mutex);
            delete g_handle;                     // 释放对象
            g_handle = nullptr;            // 将调用者的指针置空
        }
    ...
    DEPLOYAI_LIB_API float onnx_pipeline_process_file(
        ...
        if (!g_handle || !g_handle->cpp_pipeline || !ply_path || !output_path) {
            return -1.0f; // 参数错误
        }

        try {
            // 1. 重新加载点云数据（为了保留原始 PLY 的所有属性）
            PointCloud pc = load_data_ply(std::string(ply_path));
            if (pc.num_points == 0) {
                std::lock_guard<std::mutex> lock(g_mutex);
                g_handle->last_error = "Loaded point cloud is empty: " + std::string(ply_path);
                return -2.0f;
            }

            // 2. 执行推理
            auto result = g_handle->cpp_pipeline->process_pointcloud(pc);

            // 3. 写入带标签的 PLY
            // 注意：这里我们复用了原有的 write_annotated_ply，
            // 但请注意原函数会覆盖文件且只写入固定字段。
            // 如果需要保留原始字段，建议扩展 write_annotated_ply 或在此处实现追加逻辑。
            write_annotated_ply(std::string(output_path), pc, result.predictions);
    ...
    DEPLOYAI_LIB_API float onnx_pipeline_process_inmemory(
        ...
        if (!g_handle || !g_handle->cpp_pipeline || !coord_x || !coord_y || !coord_z
    ...
    DEPLOYAI_LIB_API float rdp_ai_infer_and_update(
        ...
        try {
            ...
            convert_cdi_to_pointcloud_v2(
                ...
                rcs_off, snr_off, v_off,
                pc
            );
            if (pc.num_points == 0) {
                std::lock_guard<std::mutex> lock(g_mutex);
                g_handle->last_error = "Loaded point cloud is fail.";
                return -2.0f;
            }
            //保存初始雷达点云
            write_annotated_ply(std::string("D:/ProgramData/hpenet_onnx/radarfull/raw/baocun.ply"), pc, pc.label);
            ...
            //保存预测结果雷达点云
            write_annotated_ply(std::string("D:/ProgramData/hpenet_onnx/radarfull/output/baocun.ply"), pc, result.predictions);
    ...
    DEPLOYAI_LIB_API const char* onnx_get_last_error(void) {
        if (!g_handle) return "Invalid handle (null)";
        std::lock_guard<std::mutex> lock(g_mutex);
        return g_handle->last_error.c_str();
    }
