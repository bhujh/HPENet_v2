...
/// 封装 IExecutionContext + 动态形状 + enqueueV2。
/// 兼容 TensorRT 8.2 - 10.x (Binding API)。
///
...
public:
    ...
    ///   3. 分配/重用输出 buffer → set_tensorAddress("output", ...)
    ///   4. enqueueV2(bindings, stream)
    ///
...
private:
    ...
    /// 绑定名称 → 索引 映射 (用于 enqueueV2)
    std::unordered_map<std::string, int> binding_idx_map_;

    /// 绑定名称 → GPU 地址 映射 (用于 enqueueV2)
    std::unordered_map<std::string, void*> binding_addrs_;
};
