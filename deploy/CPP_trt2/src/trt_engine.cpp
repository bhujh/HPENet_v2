...
// ══════════════════════════════════════════════════════════════════════════════
//
// 当前代码兼容 TensorRT 8.2 - 10.x。
// TRT 8.2-8.4 使用 Binding API (getNbBindings / getBindingName / bindingIsInput /
// getBindingDimensions / getBindingDataType / setBindingDimensions / enqueueV2)。
// TRT 8.5+ 新增 Tensor API (getNbIOTensors / getIOTensorName / getTensorShape /
// getTensorDataType / setTensorAddress / enqueueV3)，但 Binding API 依然保留至 TRT 10.x。
// 本文件使用 Binding API 以确保最大兼容范围。
//
...
TrEngine::TrEngine(const std::string& engine_path, TrLogger& logger) {
    ...
    // 4. 遍历 I/O 张量（TensorRT 8.2 Binding API, index-based）
