...
- [ ] 1. 项目脚手架：创建 deploy/CPP/ 目录结构和 CMakeLists.txt
...
- [ ] 2. Logger 工具类：实现 TrLogger + CUDA/TRT 错误检查宏
...
- [ ] 3. 通用类型定义：PointCloud, FeatureStats, Config, InferenceResult 结构体
...
- [ ] 4. PLY 读取器：基于 tinyply 3.0 实现 PointCloud 加载
...
- [ ] 5. 特征统计 JSON 读取器 + pth→JSON Python 转换脚本
...
- [ ] 6. 黄金数据生成器：Python 脚本生成 CUDA kernel 测试向量
...
- [ ] 7. 随机数工具：实现 numpy.random.seed(100) 兼容的 MT19937 PRNG
...
- [ ] 8. FNV64-1A 哈希 CUDA kernel：实现与测试（对比黄金数据）
...
- [ ] 9. 体素化 CUDA kernel：完整体素化 + 子云分割 + 测试
...
- [ ] 10. 特征归一化 + 子云预处理 CUDA kernel：实现与测试
...
- [ ] 11. 子云填充和分块逻辑：padding + chunking 实现与测试
...
- [ ] 12. Scatter-mean CUDA kernel：实现与测试（对比黄金数据）
...
- [ ] 13. GPU 内存 RAII 管理器：CudaBuffer, CudaStream, DeviceGuard
...
- [ ] 14. TRT 引擎加载器：反序列化 .engine 文件 + I/O 张量信息查询
...
- [ ] 15. TRT 推理执行器：ExecutionContext + 动态形状 + enqueueV3
...
- [ ] 18. 性能基准模块：延迟/吞吐量测量 + 与 Python 对比
...
- [ ] F1. **方案合规审查** — `oracle`
  读取方案端到端。对每个 "Must Have": 验证实现存在。对每个 "Must NOT Have": 搜索代码库中是否有禁止模式。检查 .omo/evidence/ 下的证据文件是否存在。对比交付物与方案。
  输出: `Must Have [N/N] | Must NOT Have [N/N] | Tasks [N/18] | VERDICT: APPROVE/REJECT`

- [ ] F2. **代码质量审查** — `unspecified-high`
  编译检查：`cmake --build build --target all`。运行完整测试套件: `ctest --output-on-failure`。审查所有修改文件: `new`/`delete` vs CudaBuffer 使用, raw cudaMalloc 使用, 未检查的 CUDA/TRT 错误码, 内存泄漏, 未使用的 include, 循环依赖, 注释掉的代码。
  输出: `Build [PASS/FAIL] | Tests [N pass/N fail] | Files [N clean/N issues] | VERDICT`

- [ ] F3. **实地手动 QA** — `unspecified-high`
  从干净状态开始。执行每个任务的所有 QA 场景 — 精确步骤，捕获证据。执行跨任务集成测试: 端到端 PLY→推理→输出，与 Python 对比。边界测试: 空 PLY, 单点云, 超大云, NaN 数据, FP16 engine。保存到 `.omo/evidence/final-qa/`。
  输出: `Scenarios [N/N pass] | Integration [N/N] | Edge Cases [N tested] | VERDICT`

- [ ] F4. **范围保真度检查** — `deep`
  对每个任务: 读取 "What to do"，读取实际 diff (git log/diff)。验证 1:1 — 方案中指定的全部已构建（无遗漏），方案外的全部未构建（无蔓延）。检查 "Must NOT do" 合规。检测跨任务污染。标记未说明的变更。
